...
class ChapterPlan(BaseModel):
    ...
    is_adult: bool = Field(
        default=True,
        description="ВСЕГДА true — это adult-новелла, откровенная сцена есть в "
        "КАЖДОЙ главе без исключений. Не ставь false.",
    )
    adult_note: str = Field(
        default="",
        description="Кто участвует в сцене(ах) (можно НЕСКОЛЬКО персонажей — "
        "смешанные/групповые разрешены), динамика, повод входа и СКОЛЬКО сцен "
        "в главе, если больше одной. Для художника и Бота 6.",
    )
...
class State(TypedDict, total=False):
    ...
    manual_chapters: bool       # главы заданы вручную → не генерить/не править структуру
    skip_structure_check: bool  # одноразовый пропуск проверки структуры
